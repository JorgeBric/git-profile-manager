...
function select_project {
  ...
  if [[ ${#repos[@]} -eq 0 ]]; then
    ...
    while true; do
      ...
      case "$REPLY" in
        0) _commit_reminder_if_dirty; echo "🌄 Exiting selector."; return 0 ;;
  ...
  while true; do
    ...
    if [[ "$REPLY" == "r" || "$REPLY" == "R" ]]; then
      _commit_reminder_if_dirty; cd "$GITHUB_ROOT" || echo "⚠️ Could not cd to root."
    ...
    if [[ "$REPLY" == "/" ]]; then filter=""; continue; fi
    if [[ "$REPLY" == /* ]]; then filter="${REPLY#/}"; continue; fi
    if [[ "$REPLY" =~ ^[0-9]+$ ]]; then
      local idx=$((REPLY - 1))
      if [[ $idx -ge 0 && $idx -lt ${#shown[@]} ]]; then
        ...
        echo "⚠️ Invalid number."; continue
      fi
    fi
...
function account-menu {
  while true; do
    ...
    case "$choice" in
      0)
        echo " "; echo "✅ Exiting profile selection."; echo " "; return 0 ;;
      1)
        echo " "
        ...
        git_set_personal
        while true; do
          select_project "$GITHUB_ROOT_PERSONAL"; rc=$?
          [[ $rc -eq 99 ]] && break || return 0
        done ;;
      2)
        echo " "
        ...
        git_set_work
        while true; do
          select_project "$GITHUB_ROOT_WORK"; rc=$?
          [[ $rc -eq 99 ]] && break || return 0
        done ;;
      *)
        echo " "; echo "⚠️ Please choose 0, 1, or 2." ;;
    esac
  done
}
...
projects() {
  local root=""
  if [[ "$PWD" == "$GITHUB_ROOT_PERSONAL"* ]]; then root="$GITHUB_ROOT_PERSONAL"
  elif [[ "$PWD" == "$GITHUB_ROOT_WORK"* ]]; then root="$GITHUB_ROOT_WORK"
  elif [[ "$GIT_ACCOUNT" == *"PERSONAL"* ]]; then root="$GITHUB_ROOT_PERSONAL"
  elif [[ "$GIT_ACCOUNT" == *"WORK"* ]]; then root="$GITHUB_ROOT_WORK"
  else
    echo "⚠️ Not in a GitHub root and no profile selected."
    account-menu; return
  fi
  while true; do
    select_project "$root"; rc=$?
    [[ $rc -eq 99 ]] && { account-menu; return; } || return
  done
}
